...
describe('Utility Functions', () => {
  ...
  describe('isValidSubmoltName', () => {
    ...
    it('rejects invalid names', () => {
      ...
      expect(isValidSubmoltName('invalid-name')).toBe(false); // invalid char
    });
  });

  describe('isValidApiKey', () => {
    it('validates correct API keys', () => {
      expect(isValidApiKey('agentsmem_abcdefghij1234567890')).toBe(true);
    });

    it('rejects invalid API keys', () => {
      expect(isValidApiKey('invalid_key')).toBe(false);
      expect(isValidApiKey('agentsmem_short')).toBe(false);
    });
  });
